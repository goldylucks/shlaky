...
let ResourceStore = (_class = (_temp = class ResourceStore extends _Store.default {
  constructor(...args) {
    ...
    this.add = (0, _mobx.action)(this.addActionName, async toAdd => {
      ...
      this.debug('Added', item); // replace optimistic update item with server response

      this._all = this._all.map(_item => _item.optimisticId === optimisticId ? data[this.itemKey] : _item);
